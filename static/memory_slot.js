...
document.addEventListener("DOMContentLoaded", function () {
    // Fetch memory slot data from the Flask route
    fetch('/api/memory-slot-data')
        .then(response => response.json())
        .then(data => {
            // Call a function to populate the table with the received data
            populateMemorySlotTable(data);
        })
        .catch(error => {
            console.error('Error fetching memory slot data:', error);
        });

    // Function to populate the table
    function populateMemorySlotTable(data) {
        const tableBody = document.getElementById('memory-slot-data');

        // Clear existing table rows
        tableBody.innerHTML = '';

        // Iterate through memory slot data and create table rows
        data.forEach(slot => {
